...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (!cap.ok) {
      return new Response(JSON.stringify({ ok: false, reason: 'amazon_daily_cap', usedToday: cap.used, dailyLimit: AMAZON_DAILY_CAP }), {
        status: 429,
    ...
    if (cached) {
      if (uid) ctx.waitUntil(logUsage(env as any, uid, { endpoint: '/amazon-search', tier, featureTag: `amazon_${type}`, priority: 'experience', mode: 'cached', ok: true, latencyMs: Date.now() - started, status: 200, cacheHit: true }));
    ...
    const payload = {
      ...
      ItemCount: 12,
      Resources: ['ItemInfo.Title', 'ItemInfo.ByLineInfo', 'Images.Primary.Medium', 'Images.Primary.Small'],
    };
    ...
    if (!amazonRes.ok) {
      if (uid) ctx.waitUntil(logUsage(env as any, uid, { endpoint: '/amazon-search', tier, featureTag: `amazon_${type}`, priority: 'experience', mode: 'full', ok: false, latencyMs: Date.now() - started, status: amazonRes.status, note: raw.slice(0, 120) }));
    ...
    if (uid) ctx.waitUntil(logUsage(env as any, uid, { endpoint: '/amazon-search', tier, featureTag: `amazon_${type}`, priority: 'experience', mode: 'full', ok: true, latencyMs: Date.now() - started, status: 200, cacheHit: false }));
